...
exports.organizationsGetUsage = organizationsGetUsage;
 ...
 */
function organizationsGetUsage(client, options) {
    return new async_js_1.APIPromise($do(client, options));
}
async function $do(client, options) {
    const path = (0, url_js_1.pathToFunc)("/v1/organization/usage")();
    ...
    const [result] = await M.match(M.json("2XX", shared.OrganizationUsageResponse$inboundSchema), M.jsonErr("4XX", errors.ErrorT$inboundSchema), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
